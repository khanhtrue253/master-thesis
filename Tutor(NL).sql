With user_table as(
  ...
  SELECT u._id,
    ...
    u.tutor.search_modifiers.weight AS tutor_manual_weight,
    u.tutor.search_modifiers.intake_date AS tutor_intake_date,
...
Select _id AS tutor,
  ...
  country,
  state,
  tutor_accepts_new_students,
  tutor_max_travel_distance,
  tutor_can_teach_online,
  tutor_has_availability,
  tutor_course_offerings_count,
  ta.tutor_category,
  tutor_manual_weight,
  tutor_intake_date,
  latitude,
  longitude
From user_table LEFT JOIN (select DISTINCT tutor, max(tutor_category) AS tutor_category from tutoring_booking.appointments group by tutor) ta on user_table._id = ta.tutor
where role = 'Tutor' AND country = 'nl' AND state in ('Active', 'Tutoring', 'Disabled', 'Chatting', 'Introducing') ANd ta.tutor_category IS NOT NULL;
